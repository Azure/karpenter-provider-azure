...
func (o *Options) AddFlags(fs *coreoptions.FlagSet) {
	...
	fs.StringVar(&o.PodSubnetID, "pod-subnet-id", env.WithDefaultString("POD_SUBNET_ID", ""), "The subnet ID pods get their IPs from, for clusters using Azure CNI with pod subnet. Must be a valid ARM resource ID for a subnet in the same virtual network as --vnet-subnet-id. Only applies to newly provisioned nodes; nodes created before this was set keep their existing networking until they are replaced.")
	fs.StringVar(&o.PodIPAllocationMode, "pod-ip-allocation-mode", env.WithDefaultString("POD_IP_ALLOCATION_MODE", ""), "The pod IP allocation mode for clusters using Azure CNI with pod subnet. Must be DynamicIndividual or StaticBlock when --pod-subnet-id is set.")
