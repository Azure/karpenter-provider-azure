...
func (o *Options) AddFlags(fs *coreoptions.FlagSet) {
	...
	fs.BoolVar(&o.BatchCreationEnabled, "batch-creation-enabled", env.WithDefaultBool("BATCH_CREATION_ENABLED", true), "If set to true, enables batch creation for AKS Machine API to optimize scaling performance. Only used on AKS machine API provision mode.")
	fs.BoolVar(&o.ListPollerEnabled, "list-poller-enabled", env.WithDefaultBool("LIST_POLLER_ENABLED", true), "If set to true, enables cache-based list polling for AKS Machine API to reduce individual GET requests and API throttling. Only used on AKS machine API provision mode.")
