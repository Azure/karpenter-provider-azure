...
func (o *Options) AddFlags(fs *coreoptions.FlagSet) {
	...
	fs.DurationVar(&o.ProviderBatchIdleDuration, "provider-batch-idle-duration", env.WithDefaultDuration("PROVIDER_BATCH_IDLE_DURATION", time.Second), "How long to wait after last request before firing, using Go duration syntax such as 1s. Only used on provision mode aksmachineapiheaderbatch.")
	fs.DurationVar(&o.ProviderBatchMaxDuration, "provider-batch-max-duration", env.WithDefaultDuration("PROVIDER_BATCH_MAX_DURATION", 5*time.Second), "Hard cap on batch wait time, using Go duration syntax such as 5s. Only used on provision mode aksmachineapiheaderbatch.")
