...
type Options struct {
	...
	BatchIdleTimeoutMS   int    `json:"batchIdleTimeoutMS,omitempty"`   // Idle timeout in milliseconds for batch accumulation (default 100ms). Only used when batch creation is enabled.
	BatchMaxTimeoutMS    int    `json:"batchMaxTimeoutMS,omitempty"`    // Maximum timeout in milliseconds for batch accumulation (default 1000ms). Only used when batch creation is enabled.
...
func (o *Options) AddFlags(fs *coreoptions.FlagSet) {
	...
	fs.BoolVar(&o.BatchCreationEnabled, "batch-creation-enabled", env.WithDefaultBool("BATCH_CREATION_ENABLED", false), "If set to true, enables batch creation for AKS Machine API to optimize scaling performance. Only used on AKS machine API provision mode.")
	fs.IntVar(&o.BatchIdleTimeoutMS, "batch-idle-timeout-ms", env.WithDefaultInt("BATCH_IDLE_TIMEOUT_MS", 100), "Idle timeout in milliseconds for batch accumulation. Only used when batch creation is enabled.")
	fs.IntVar(&o.BatchMaxTimeoutMS, "batch-max-timeout-ms", env.WithDefaultInt("BATCH_MAX_TIMEOUT_MS", 1000), "Maximum timeout in milliseconds for batch accumulation. Only used when batch creation is enabled.")
