...
type Options struct {
	...
	ComputeRecommendationMode string `json:"computeRecommendationMode,omitempty"` // Controls compute recommendation API behavior: "disabled", "log" (default), or "enabled".
...
func (o *Options) AddFlags(fs *coreoptions.FlagSet) {
	...
	// TODO: Fix this to disabled before merge (need to get RBAC problem sorted out first)
	fs.StringVar(&o.ComputeRecommendationMode, "compute-recommendation-mode", env.WithDefaultString("COMPUTE_RECOMMENDATION_MODE", consts.ComputeRecommendationModeLog), "Controls compute recommendation API behavior: 'disabled' (no API calls), 'log' (call and log only), or 'enabled' (use for allocation).")
