...
func (o *Options) AddFlags(fs *coreoptions.FlagSet) {
	...
	fs.DurationVar(&o.ProviderBatchIdleDuration, "provider-batch-idle-duration", env.WithDefaultDuration("PROVIDER_BATCH_IDLE_DURATION", time.Second), "Idle duration for provider batch accumulation. Use Go duration format such as 1s or 500ms. Only used on provision mode aksmachineapiheaderbatch.")
	fs.DurationVar(&o.ProviderBatchMaxDuration, "provider-batch-max-duration", env.WithDefaultDuration("PROVIDER_BATCH_MAX_DURATION", 5*time.Second), "Maximum duration for provider batch accumulation. Use Go duration format such as 1s or 500ms. Only used on provision mode aksmachineapiheaderbatch.")
