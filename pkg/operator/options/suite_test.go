...
var _ = Describe("Options", func() {
	...
	Context("Validation", func() {
		...
		It("should fail validation when matching subnet IDs differ only by casing", func() {
			err := opts.Parse(
				fs,
				"--cluster-name", "my-name",
				"--cluster-endpoint", "https://karpenter-000000000000.hcp.westus2.staging.azmk8s.io",
		...
		DescribeTable("should fail validation when pod-subnet-id is set on an unsupported provision mode", func(provisionMode string, extraArgs ...string) {
			args := []string{
				"--cluster-name", "my-name",
				"--cluster-endpoint", "https://karpenter-000000000000.hcp.westus2.staging.azmk8s.io",
				"--kubelet-bootstrap-token", "flag-bootstrap-token",
			...
			Expect(err).To(MatchError(ContainSubstring(fmt.Sprintf("pod-subnet-id is not supported with provision-mode '%s'", provisionMode))))
		},
			Entry("bootstrapping client", consts.ProvisionModeBootstrappingClient, "--nodebootstrapping-server-url", "https://nodebootstrapping-server-url"),
			Entry("AKS machine API", consts.ProvisionModeAKSMachineAPI, "--aks-machines-pool-name", "testmpool", "--use-sig", "--sig-subscription-id", "12345678-1234-1234-1234-123456789012"),
			Entry("AKS machine API header batch", consts.ProvisionModeAKSMachineAPIHeaderBatch, "--aks-machines-pool-name", "testmpool", "--use-sig", "--sig-subscription-id", "12345678-1234-1234-1234-123456789012"),
		)
	...
	Context("AKS Machine API", func() {
		...
		It("should succeed with provision-mode aksmachineapiheaderbatch", func() {
			err := opts.Parse(
				fs,
				"--cluster-name", "my-name",
				"--cluster-endpoint", "https://karpenter-000000000000.hcp.westus2.staging.azmk8s.io",
