...
var _ = Describe("Options", func() {
	...
	Context("Validation", func() {
		...
		It("should reject fleet provision mode when use-sig is enabled", func() {
			err := opts.Parse(
				fs,
				"--cluster-name", "my-name",
				"--cluster-endpoint", "https://karpenter-000000000000.hcp.westus2.staging.azmk8s.io",
			...
			Expect(err).To(MatchError(ContainSubstring("use-sig is not supported when provision-mode is fleet")))
		})
