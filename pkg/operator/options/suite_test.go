	...
	Context("Validation", func() {
		...
		It("should fail if additional-tags is malformed", func() {
			err := opts.Parse(
				...
				"--vnet-guid", "a519e60a-cac0-40b2-b883-084477fe6f5c",
				"--node-resource-group", "my-node-rg",
				"--additional-tags", "key1=value1=value2",
			)
			Expect(err).To(MatchError(ContainSubstring("invalid key=value pair: \"key1=value1=value2\"")))
		})
		It("should fail if additional-tags has duplicate keys", func() {
			...
			Expect(err).To(MatchError(ContainSubstring("additional-tags key \"KEY1\" is not unique (case-insensitive). Duplicate key found")))
		})
