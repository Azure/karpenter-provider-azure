...
func (Windows2022) DefaultImages(useSIG bool, fipsMode *v1beta1.FIPSMode, trustedLaunch bool, kataEnabled bool) []types.DefaultImageOutput {
	if lo.FromPtr(fipsMode) == v1beta1.FIPSModeFIPS || trustedLaunch || kataEnabled {
...
func (Windows2025) DefaultImages(useSIG bool, _ *v1beta1.FIPSMode, trustedLaunch bool, kataEnabled bool) []types.DefaultImageOutput {
	// Kata VM isolation is Linux-only, so Windows advertises no images for it.
	if kataEnabled {
		return []types.DefaultImageOutput{}
	}
	if trustedLaunch {
