...
func TestWindows_DefaultImages(t *testing.T) {
	...
	images := imagefamily.Windows2022{}.DefaultImages(true, nil, false, false)
	...
	images2025 := imagefamily.Windows2025{}.DefaultImages(true, nil, false, false)
	...
	g.Expect(imagefamily.Windows2022{}.DefaultImages(false, nil, false, false)).To(BeEmpty())

	// Windows2022 does not support FIPS.
	g.Expect(imagefamily.Windows2022{}.DefaultImages(true, lo.ToPtr(v1beta1.FIPSModeFIPS), false, false)).To(BeEmpty())

	// Windows2025 requires FIPS in the AKS RP, so its images remain available in FIPS mode.
	g.Expect(imagefamily.Windows2025{}.DefaultImages(true, lo.ToPtr(v1beta1.FIPSModeFIPS), false, false)).To(HaveLen(2))

	trustedLaunchImages := imagefamily.Windows2025{}.DefaultImages(true, nil, true, false)
	...
	g.Expect(imagefamily.Windows2022{}.DefaultImages(true, nil, true, false)).To(BeEmpty())

	// Kata VM isolation is Linux-only, so neither Windows family advertises images for it.
	g.Expect(imagefamily.Windows2022{}.DefaultImages(true, nil, false, true)).To(BeEmpty())
	g.Expect(imagefamily.Windows2025{}.DefaultImages(true, nil, false, true)).To(BeEmpty())
}
...
func TestWindows_BootstrapMethodsUnsupported(t *testing.T) {
	...
	_, _, err = w.CustomScriptsNodeBootstrapping(nil, nil, nil, nil, nil, "", "", nil, nil, nil, nil, nil, nil, nil, nil).GetCustomDataAndCSE(context.Background())
	g.Expect(err).To(HaveOccurred())
	g.Expect(err.Error()).To(ContainSubstring("aksmachineapi"))
}
