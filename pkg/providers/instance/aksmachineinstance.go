...
func NewAKSMachineProvider(
	...
	provider := &DefaultAKSMachineProvider{
		...
		machineListCache:                aksmachinepoller.NewMachineListCache(ctx, 30*time.Second, azClient.AKSMachinesClient(), 5*time.Second, clusterResourceGroup, clusterName, aksMachinesPoolName),
...
func (p *DefaultAKSMachineProvider) getMachine(ctx context.Context, aksMachineName string) (*armcontainerservice.Machine, error) {
	...
	aksMachine, err := p.machineListCache.Get(ctx, aksMachineName)
	if err == nil {
