...
// assert that DefaultVMProvider implements Provider interface
...
func (p *DefaultVMProvider) BeginCreate(
	...
	vm := vmPromise.VM
	zone, err := utils.GetAKSZoneFromVM(vm)
	if err != nil {
...
func newVMObject(opts *createVMOptions) *armcompute.VirtualMachine {
	...
	vm := &armcompute.VirtualMachine{
		...
		Zones: utils.GetARMZonesFromAKSZone(opts.Zone),
...
func GetCapacityTypeFromVM(instance *armcompute.VirtualMachine) string {
	if instance != nil && instance.Properties != nil && instance.Properties.Priority != nil {
		return VMPriorityToKarpCapacityType[*instance.Properties.Priority]
	}
	return ""
}
