...
func handleZonalAllocationFailureError(ctx context.Context, provider *DefaultProvider, instanceType *corecloudprovider.InstanceType, zone, capacityType string, err error) error {
	...
	// TODO mumanski - find a "nicer" (and safer) way of retrieving the values
	skuCPUCount, err := strconv.ParseInt(instanceType.Requirements.Get(v1beta1.LabelSKUCPU).Values()[0], 10, 64)
	if err != nil {
		skuCPUCount = 0 // fallback to 0 if parsing fails, but this should not happen
	}
	skuVMFamily := instanceType.Requirements.Get(v1beta1.LabelSKUFamily).Values()[0]
	skuVMFamilyVersion := instanceType.Requirements.Get(v1beta1.LabelSKUVersion).Values()[0]
	provider.unavailableOfferings.MarkFamilyUnavailableWithTTL(ctx, skuVMFamily+skuVMFamilyVersion, karpv1.CapacityTypeSpot, zone, skuCPUCount, AllocationFailureTTL)
	provider.unavailableOfferings.MarkFamilyUnavailableWithTTL(ctx, skuVMFamily+skuVMFamilyVersion, karpv1.CapacityTypeOnDemand, zone, skuCPUCount, AllocationFailureTTL)
