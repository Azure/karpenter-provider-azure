...
// setKataNodeLabel stamps the Kata (Pod Sandboxing) node label when the NodeClass requests it,
// matching the requirement advertised by the instance type provider (see computeRequirements).
func setKataNodeLabel(labels map[string]string, nodeClass *v1beta1.AKSNodeClass) {
	if !nodeClass.IsKataEnabled() {
		return
	}
	labels[v1beta1.AKSLabelKataVMIsolation] = "true"
}
...
func Get(
	...
	if (nodeClass.Spec.PodSubnetID == nil) != (nodeClass.Spec.PodIPAllocationMode == nil) {
		return nil, fmt.Errorf("podSubnetID and podIPAllocationMode must be specified together")
	}
...
func setNetworkLabels(ctx context.Context, labels map[string]string, nodeClass *v1beta1.AKSNodeClass, subnetID, kubernetesVersion string) error {
	...
	if err := podSubnetComponents.ValidateLabelValues(); err != nil {
		return fmt.Errorf("podSubnetID: %w", err)
	}
	if err := nodeSubnetComponents.ValidateLabelValues(); err != nil {
		return fmt.Errorf("vnetSubnetID: %w", err)
	}
	...
	var podNetworkType string
	switch podIPAllocationMode := nodeClass.GetPodIPAllocationMode(opts.PodIPAllocationMode); {
	case strings.EqualFold(podIPAllocationMode, consts.PodIPAllocationModeDynamicIndividual):
		podNetworkType = consts.PodNetworkTypeVNet
	case strings.EqualFold(podIPAllocationMode, consts.PodIPAllocationModeStaticBlock):
		podNetworkType = consts.PodNetworkTypeVNetBlock
	default:
		return fmt.Errorf("unsupported pod IP allocation mode %q", podIPAllocationMode)
	}
