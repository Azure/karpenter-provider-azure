	...
	Context("ProvisionMode = AKSScriptless", func() {
		...
		Context("ImageReference", func() {
			It("should use shared image gallery images when options are set to UseSIG", func() {
				...
				statusController := status.NewController(env.Client, azureEnv.KubernetesVersionProvider, azureEnv.ImageProvider, env.KubernetesInterface, azureEnv.SubnetsAPI, nil)
