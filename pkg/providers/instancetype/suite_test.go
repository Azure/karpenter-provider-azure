...
var _ = Describe("InstanceType Provider", func() {
	...
	Context("Ephemeral Disk", func() {
		...
		It("should provide right placement for b20ms", func() {
			skus := fake.ResourceSkus["southcentralus"]
			StandardB20ms := compute.ResourceSku{}
			for _, sku := range skus {
				if lo.FromPtr(sku.Name) == "Standard_B20ms" {
					StandardB20ms = sku
					break
				}
			}

			sku := skewer.SKU(StandardB20ms)
			sizeGB := instancetype.MaxEphemeralOSDiskSizeGB(&sku)
			Expect(sizeGB).To(BeNumerically("~", 171.79869184, 0.00001))
		})
	})
