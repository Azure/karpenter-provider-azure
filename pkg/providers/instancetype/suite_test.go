	...
	Context("ProvisionMode = AKSScriptless", func() {
		...
		Context("Cache invalidation with LocalDNS", func() {
			It("should return different instance type lists when LocalDNS mode changes", func() {
				...
				instanceTypesDisabled, err := azureEnv.InstanceTypesProvider.ListAKS(ctx, nodeClassDisabled, nil)
				Expect(err).ToNot(HaveOccurred())
				...
				instanceTypesEnabled, err := azureEnv.InstanceTypesProvider.ListAKS(ctx, nodeClassEnabled, nil)
				Expect(err).ToNot(HaveOccurred())
		...
		// tests are shared in pkg/cloudprovider/suite_offerings_test.go via runShared*Tests
	})
	...
	Context("Provider List", func() {
		...
		Context("Filtering GPU SKUs AzureLinux", func() {
			...
			BeforeEach(func() {
				...
				instanceTypes, err = azureEnv.InstanceTypesProvider.ListAKS(ctx, nodeClassAZLinux, nil)
				Expect(err).ToNot(HaveOccurred())
			})
		...
		Context("Filtering by Encryption at Host", func() {
			...
			Context("when encryption at host is enabled", func() {
				BeforeEach(func() {
					...
					instanceTypes, err = azureEnv.InstanceTypesProvider.ListAKS(ctx, nodeClassWithEncryption, nil)
					Expect(err).ToNot(HaveOccurred())
				})
			...
			Context("when encryption at host is disabled or not set", func() {
				It("should include SKUs regardless of encryption at host support", func() {
					...
					instanceTypes, err = azureEnv.InstanceTypesProvider.ListAKS(ctx, nodeClassWithoutEncryption, nil)
					Expect(err).ToNot(HaveOccurred())
