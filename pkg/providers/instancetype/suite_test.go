...
var _ = Describe("InstanceType Provider", func() {
	...
	// BootstrappingClient tests moved to pkg/cloudprovider/suite_features_test.go
	...
	Context("ProvisionMode = AKSScriptless", func() {
		// Subnet tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)
		// VM Creation Failures moved to pkg/cloudprovider/suite_offerings_test.go (AKSScriptless-specific)

		// Custom DNS tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)
		// additional-tags tests moved to pkg/cloudprovider/suite_features_test.go (runSharedAdditionalTagsTests)
		...
		Context("Ephemeral Disk", func() {
			...
			// Placement + NvmeDisk tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)

		})

		// Custom DNS tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)

		// KubeletConfig and KubeletConfig on kubenet tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)

		// ImageReference + ImageProvider + Image Family tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)

		// Instance Types tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)

		// GPU Workloads + Nodes tests moved to pkg/cloudprovider/suite_features_test.go (runSharedGPUTests)

		// Bootstrap tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)

		// LoadBalancer tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)

		// Zone-aware provisioning tests moved to pkg/cloudprovider/suite_offerings_test.go (runSharedZoneAwareProvisioningTests)
		// VM-specific placement scope tests moved to AKSScriptless-specific context in suite_offerings_test.go

		// CloudProvider Create Error Cases + Unavailable Offerings VM-specific moved to pkg/cloudprovider/suite_offerings_test.go (AKSScriptless-specific)
	})
	...
	Context("Provider List", func() {
		...
		// Basic tests moved to pkg/cloudprovider/suite_features_test.go (AKSScriptless-specific)
	})
...
// ExpectKubeletFlagsPassed and ExpectKubeletNodeLabelsPassed moved to pkg/test/expectations/expectations.go
...
// ExpectKubeletNodeLabelsInCustomData and ExpectKubeletNodeLabelsNotInCustomData moved to pkg/test/expectations/expectations.go
