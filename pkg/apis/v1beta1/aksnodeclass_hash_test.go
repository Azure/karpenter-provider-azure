...
func TestHash(t *testing.T) {
	...
	t.Run("should match static hash on field value change", func(t *testing.T) {
		tests := []struct {
			...
			{"ArtifactStreaming.Enabled", "15355387647114481444", v1beta1.AKSNodeClass{Spec: v1beta1.AKSNodeClassSpec{ArtifactStreaming: &v1beta1.ArtifactStreaming{Enabled: lo.ToPtr(true)}}}},
		}
	...
	t.Run("should change hash when static fields are updated", func(t *testing.T) {
		tests := []struct {
			...
			{"ArtifactStreaming.Enabled", v1beta1.AKSNodeClass{Spec: v1beta1.AKSNodeClassSpec{ArtifactStreaming: &v1beta1.ArtifactStreaming{Enabled: lo.ToPtr(true)}}}},
		}
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				nodeClass := newHashTestNodeClass()
