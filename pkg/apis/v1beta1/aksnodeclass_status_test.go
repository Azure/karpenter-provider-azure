...
var _ = Describe("Status, successful outcomes", func() {
	...
	It("should return conditions", func() {
		...
		Expect(conditions[0].Type).To(Equal(v1beta1.ConditionTypeImagesReady))
	...
	It("should return status conditions", func() {
		...
		Expect(conditionSet.List()).To(HaveLen(6)) // KubernetesVersionReady, SubnetReady, ImagesReady, ValidationSucceeded, LocalDNSReady, Ready
	...
	It("should return the expected errors", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(images).To(Equal([]v1beta1.NodeImage{}))
