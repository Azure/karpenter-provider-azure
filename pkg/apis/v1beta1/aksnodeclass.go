...
// +kubebuilder:validation:XValidation:message="TrustedLaunch is not supported for the selected imageFamily",rule="has(self.security) && has(self.security.trustedLaunch) && ((has(self.security.trustedLaunch.vtpm) && self.security.trustedLaunch.vtpm) || (has(self.security.trustedLaunch.secureBoot) && self.security.trustedLaunch.secureBoot)) ? (!has(self.imageFamily) || self.imageFamily != 'Ubuntu2004') : true"
