...
var _ = Describe("CEL/Validation", func() {
	...
	Context("ImageFamily, FIPSMode, and TrustedLaunch", func() {
		DescribeTable("should only accept valid ImageFamily, FIPSMode, and TrustedLaunch combinations", func(imageFamily string, fipsMode *v1alpha2.FIPSMode, trustedLaunch bool, expected bool) {
			...
			Entry("generic Ubuntu when FIPSMode is explicitly FIPS and TrustedLaunch is enabled should fail", v1alpha2.UbuntuImageFamily, &v1alpha2.FIPSModeFIPS, true, false),
			...
			Entry("unspecified ImageFamily (defaults to Ubuntu) when FIPSMode is explicitly FIPS and TrustedLaunch is enabled should fail", "", &v1alpha2.FIPSModeFIPS, true, false),
		)
	})
