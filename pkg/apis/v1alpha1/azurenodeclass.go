...
var (
	...
	AnnotationInPlaceUpdateHash         = apis.Group + "/azurenodeclass-in-place-update-hash"
...
type AzureNodeClassSpec struct {
	...
	// image versions (/SharedGalleries/.../...).
	// Required for AzureNodeClass — the user must provide their own image.
	// +kubebuilder:validation:Pattern=`(?i)^\/[a-zA-Z]`
	// +required
	ImageID *string `json:"imageID"`
	// imageFamily declares the OS family of the custom image specified by imageID.
	// This is used for GPU instance type compatibility filtering — different GPU
	// SKUs require different OS families for driver support (e.g., NVIDIA GPUs
	// require Ubuntu-family images). If not specified, GPU instance types are
	// excluded from scheduling unless explicitly pinned via the instanceTypes field.
	// +kubebuilder:validation:Enum={Ubuntu,AzureLinux}
	// +optional
	...
	// maxPods is the maximum number of pods that can run on a worker node.
	// This directly affects Karpenter's scheduling decisions (pod capacity per instance type).
	// If not specified, defaults to 110 (the standard Kubernetes default).
...
func (in *AzureNodeClass) GetImageFamily() *string {
