...
var _ = Describe("Validation Reconciler", func() {
	...
	Context("netIPv4IPLocalPortRange validation", func() {
		It("should pass with valid port range", func() {
		...
		It("should fail when first port is below 1024", func() {
			nodeClass.Spec.LinuxOSConfig = &v1beta1.LinuxOSConfiguration{
				Sysctls: &v1beta1.SysctlConfiguration{
			...
			Expect(condition.Reason).To(Equal(status.NetIPv4IPLocalPortRangeInvalid))
			Expect(condition.Message).To(ContainSubstring("first port must be in [1024, 60999]"))
		})

		It("should fail when last port exceeds 65535", func() {
			nodeClass.Spec.LinuxOSConfig = &v1beta1.LinuxOSConfiguration{
				Sysctls: &v1beta1.SysctlConfiguration{
			...
			Expect(condition.IsFalse()).To(BeTrue())
			Expect(condition.Reason).To(Equal(status.NetIPv4IPLocalPortRangeInvalid))
			Expect(condition.Message).To(ContainSubstring("last port must be in [32768, 65535]"))
		})

		It("should fail when first port exceeds last port", func() {
			nodeClass.Spec.LinuxOSConfig = &v1beta1.LinuxOSConfiguration{
				Sysctls: &v1beta1.SysctlConfiguration{
			...
			Expect(condition.IsFalse()).To(BeTrue())
			Expect(condition.Reason).To(Equal(status.NetIPv4IPLocalPortRangeInvalid))
			Expect(condition.Message).To(ContainSubstring("first port (60000) must be <= last port (32768)"))
		})

		It("should pass when linuxOSConfig is nil", func() {
		...
		It("should pass when port range is not set", func() {
			nodeClass.Spec.LinuxOSConfig = &v1beta1.LinuxOSConfiguration{
				Sysctls: &v1beta1.SysctlConfiguration{
