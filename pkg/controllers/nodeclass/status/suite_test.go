...
var _ = BeforeSuite(func() {
	...
	controller = status.NewController(env.Client, azureEnv.KubernetesVersionProvider, azureEnv.ImageProvider, env.KubernetesInterface, env.KubernetesInterface, azureEnv.DynamicInterface, azureEnv.SubnetsAPI, azureEnv.DiskEncryptionSetsAPI, testOptions.ParsedDiskEncryptionSetID, azureEnv.InstanceTypesProvider, options.FromContext(ctx).NetworkPolicy, options.FromContext(ctx).NetworkPlugin)
})
