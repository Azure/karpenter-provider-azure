...
func NewController(
	...
	return &Controller{
		...
		localDNS:          NewLocalDNSReconciler(managedKubernetesInterface, managedDynamicInterface, kubeClient, instanceTypeProvider, instanceTypeStore, networkPolicy, networkPlugin),
	}
}
...
func (c *Controller) Register(_ context.Context, m manager.Manager) error {
	return controllerruntime.NewControllerManagedBy(m).
		...
		// with LocalDNS off.
		//
		// Filtered to spec changes: a reconcile runs every subreconciler,
		// including a node image lookup and an Azure subnet GET, and NodePool
		// status churns far more often than its spec. Every input the gate
		// reads (nodeClassRef, requirements, template labels) lives under spec,
		// so a generation bump covers them; Create and Delete are unfiltered,
		// which is what closes the startup race above.
		Watches(
			&karpv1.NodePool{},
			handler.EnqueueRequestsFromMapFunc(nodePoolToAKSNodeClass),
