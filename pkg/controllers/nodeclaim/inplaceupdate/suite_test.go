...
var _ = Describe("Unit tests", func() {
	...
	Context("calculateAKSMachinePatch", func() {
		It("should add missing tags to AKS machine when there are no existing tags", func() {
			...
			Expect(update.Properties.Tags).To(HaveKey("karpenter.azure.com_aksmachine"))
		...
		It("should not update AKS machine when tags already match", func() {
			...
			Expect(aksMachine.Properties.Tags).To(HaveLen(4))
...
var _ = Describe("In Place Update Controller", func() {
	Context("VM Instance Tests", func() {
		var vmName string
		var vm *armcompute.VirtualMachine
		...
		Context("AKS Machine Instance Tests", func() {
			...
			BeforeEach(func() {
				...
				azureEnv.SharedStores.AgentPools.Store(lo.FromPtr(aksMachinesPool.ID), *aksMachinesPool)
			})
