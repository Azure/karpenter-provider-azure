...
var _ = Describe("CloudProvider", func() {
	Context("ProvisionMode = AKSMachineAPIHeaderBatch, ManageExistingAKSMachines = false", func() {
		BeforeEach(func() {
			...
			statusController = status.NewController(env.Client, azureEnv.KubernetesVersionProvider, azureEnv.ImageProvider, env.KubernetesInterface, env.KubernetesInterface, nil, azureEnv.SubnetsAPI, azureEnv.DiskEncryptionSetsAPI, testOptions.ParsedDiskEncryptionSetID, azureEnv.InstanceTypesProvider, azureEnv.InstanceTypeStore, options.FromContext(ctx).NetworkPolicy, options.FromContext(ctx).NetworkPlugin)
