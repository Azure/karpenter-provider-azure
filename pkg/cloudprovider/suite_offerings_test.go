...
// This test file is for offering (e.g., VM size, zones) selection, quota/capacity provisioning errors, and fallback after quota exhaustion.

func installDSv3FamilyQuotaExhaustion(provisionMode provisionModeTestCase, coreQuota int32) {
	var usedCores atomic.Int32
	if provisionMode.isAKSMachineMode() {
		azureEnv.AKSMachinesAPI.AfterPollProvisioningErrorFunc = func(machine armcontainerservice.Machine) *armcontainerservice.ErrorDetail {
			if machine.Properties == nil || machine.Properties.Hardware == nil || machine.Properties.Hardware.VMSize == nil {
				return nil
			}
			current, vcpus, exceeded := exceedsDSv3FamilyQuota(&usedCores, coreQuota, lo.FromPtr(machine.Properties.Hardware.VMSize))
			if !exceeded {
				return nil
			}
			return fake.AKSMachineAPIProvisioningErrorVMFamilyQuotaExceeded(fake.Region, "standardDSv3Family", coreQuota, current, vcpus, current+vcpus)
		}
		return
	}
	azureEnv.VirtualMachinesAPI.VirtualMachineCreateOrUpdateBehavior.SetCustomTransformer(
		func(input *fake.VirtualMachineCreateOrUpdateInput) error {
			current, vcpus, exceeded := exceedsDSv3FamilyQuota(&usedCores, coreQuota, string(*input.VM.Properties.HardwareProfile.VMSize))
			if !exceeded {
				return nil
			}
...
func exceedsDSv3FamilyQuota(usedCores *atomic.Int32, coreQuota int32, vmSize string) (current int32, vcpus int32, exceeded bool) {
...
func runOfferingTests(provisionMode provisionModeTestCase) {
	Context("Create - Expected Creation Failures", func() {
		...
		It("should surface the specific failure reason on the Launched condition for a classified error", func() {
			if !provisionMode.isAKSMachineMode() {
				Skip("AKS Machine provisioning errors surface classified Launched condition reasons")
			}
			ExpectApplied(ctx, env.Client, nodePool, nodeClass)
	...
	Context("Create - large-scale provisioning with quota exhaustion", func() {
		It("should fall back to a different SKU family when quota is exhausted mid-provisioning", func() {
			...
			// Simulate quota exhaustion through the mode's create failure surface:
			// VM mode returns a CRP create error, while AKS Machine mode records a
			// Machine provisioning error. The shared behavior is that progressively
			// smaller Ds_v3 SKUs are blocked until remaining pods fall back to D_v5.
			installDSv3FamilyQuotaExhaustion(provisionMode, coreQuota)
		...
		It("should fall back to a lower-weight NodePool when the primary NodePool's only SKU family is exhausted", func() {
			...
			// Simulate quota exhaustion through the mode's create failure surface.
			// Since Ds_v3 is the only series for NodePool A, the shared behavior is
			// that exhausted Ds_v3 claims hit ICE and the scheduler falls back to B.
			installDSv3FamilyQuotaExhaustion(provisionMode, coreQuota)
