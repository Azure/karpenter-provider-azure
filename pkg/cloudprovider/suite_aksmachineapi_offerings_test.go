...
var _ = Describe("CloudProvider", func() {
	Context("ProvisionMode = AKSMachineAPIHeaderBatch", func() {
		BeforeEach(func() {
			...
			statusController = status.NewController(env.Client, azureEnv.KubernetesVersionProvider, azureEnv.ImageProvider, env.KubernetesInterface, env.KubernetesInterface, azureEnv.DynamicInterface, azureEnv.SubnetsAPI, azureEnv.DiskEncryptionSetsAPI, testOptions.ParsedDiskEncryptionSetID, azureEnv.InstanceTypesProvider, azureEnv.InstanceTypeStore, options.FromContext(ctx).NetworkPolicy, options.FromContext(ctx).NetworkPlugin)
		...
		Context("Create - CloudProvider Create Error Cases", func() {
			...
			It("should return error when NodeClass readiness is Unknown", func() {
				...
				claim, err := CreateAndWaitForPromises(ctx, cloudProvider, azureEnv, testNodeClaim2)
			...
			It("should return error when instance type resolution fails", func() {
				...
				localStatusController := status.NewController(env.Client, azureEnv.KubernetesVersionProvider, azureEnv.ImageProvider, env.KubernetesInterface, env.KubernetesInterface, azureEnv.DynamicInterface, azureEnv.SubnetsAPI, azureEnv.DiskEncryptionSetsAPI, testOptions.ParsedDiskEncryptionSetID, azureEnv.InstanceTypesProvider, azureEnv.InstanceTypeStore, options.FromContext(ctx).NetworkPolicy, options.FromContext(ctx).NetworkPlugin)
				...
				claim, err := CreateAndWaitForPromises(ctx, cloudProvider, azureEnv, testNodeClaim3)
				Expect(err).To(HaveOccurred())
				Expect(err).To(BeAssignableToTypeOf(&corecloudprovider.CreateError{}))
				Expect(claim).To(BeNil())
			...
			It("should return error when instance creation fails", func() {
				...
				claim, err := CreateAndWaitForPromises(ctx, cloudProvider, azureEnv, testNodeClaim4)
				Expect(err).To(HaveOccurred())
				Expect(err).To(BeAssignableToTypeOf(&corecloudprovider.CreateError{}))
				Expect(claim).To(BeNil())
