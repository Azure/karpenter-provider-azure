	...
	Context("ProvisionMode = AKSMachineAPIHeaderBatch", func() {
		...
		Context("Create - CloudProvider Create Error Cases", func() {
			...
			It("should return error when instance type resolution fails", func() {
				...
				localStatusController := status.NewController(env.Client, azureEnv.KubernetesVersionProvider, azureEnv.ImageProvider, env.KubernetesInterface, nil, env.KubernetesInterface, nil, azureEnv.SubnetsAPI, azureEnv.DiskEncryptionSetsAPI, testOptions.ParsedDiskEncryptionSetID, "", "")
