...
// This file contains E2E-style tests that were migrated from
// pkg/providers/instancetype/suite_test.go. They exercise the full
// provisioning path (ExpectProvisionedAndWaitForPromises / cloudProvider.Create)
// and therefore belong in the cloudprovider package.
...
// Bootstrap-mode test infrastructure (not in the base suite_test.go)
...
var _ = Describe("InstanceType E2E", func() {
	// Local setup: configure the VM-mode environments used by the tests in this file.
	// We deliberately avoid the shared setupVMMode() because these tests were originally
	// written against a specific setup (including bootstrap mode) that differs from the
	// shared adapter pattern.

	BeforeEach(func() {
		// Reset testOptions and ctx
		...
		cloudProvider = New(azureEnv.InstanceTypesProvider, azureEnv.VMInstanceProvider, azureEnv.AKSMachineProvider, events.NewRecorder(&record.FakeRecorder{}), env.Client, azureEnv.ImageProvider, azureEnv.InstanceTypeStore)
		...
		cloudProviderBootstrap = New(azureEnvBootstrap.InstanceTypesProvider, azureEnvBootstrap.VMInstanceProvider, azureEnvBootstrap.AKSMachineProvider, events.NewRecorder(&record.FakeRecorder{}), env.Client, azureEnvBootstrap.ImageProvider, azureEnv.InstanceTypeStore)
...
// Helper functions for this file (local to the cloudprovider package test)
