...
// setupAKSMachineAPIModeNoBatch configures test infrastructure for AKSMachineAPI provisioning mode
// without batch creation (direct PUT → SDK LRO poller). This tests the non-batch code path
// which is the current production default.
func setupAKSMachineAPIModeNoBatch() {
	testOptions = test.Options(test.OptionsFields{
		ProvisionMode: lo.ToPtr(consts.ProvisionModeAKSMachineAPI),
		UseSIG:        lo.ToPtr(true),
	})
	// Explicitly no batch — tests the direct Machine PUT path
