				...
				firstNodeClaim := coretest.NodeClaim(karpv1.NodeClaim{
					...
					Spec: karpv1.NodeClaimSpec{
						...
						Requirements: []karpv1.NodeSelectorRequirementWithMinValues{
							{Key: v1.LabelTopologyZone, Operator: v1.NodeSelectorOpIn, Values: []string{zones.MakeAKSLabelZoneFromARMZone(fake.Region, "1")}},
							{Key: v1.LabelInstanceTypeStable, Operator: v1.NodeSelectorOpIn, Values: []string{"Standard_D2_v2"}},
						},
					},
				})
...
func expectKubeletNodeLabelsNotInCustomData(vm *armcompute.VirtualMachine, key string, value string) {
	...
	// Extract and check KUBELET_NODE_LABELS does NOT contain the expected label
	kubeletNodeLabels := expectKubeletNodeLabelsPassed(decodedString)
	Expect(kubeletNodeLabels).ToNot(ContainSubstring(fmt.Sprintf("%s=%s", key, value)))
}

// Helper functions for LocalDNS tests
