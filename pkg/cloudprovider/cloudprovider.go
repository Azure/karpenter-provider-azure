...
func (c *CloudProvider) waitOnPromise(ctx context.Context, promise *instance.VirtualMachinePromise, nodeClaim *karpv1.NodeClaim) {
	...
	if err != nil {
		...
		if err = c.kubeClient.Delete(ctx, nodeClaim); err != nil {
			...
			if err != nil {
				log.FromContext(ctx).Error(err, "failed to delete nodeclaim, will wait for liveness TTL", "nodeClaimName", nodeClaim.Name)
			}
		}
...
func (c *CloudProvider) Delete(ctx context.Context, nodeClaim *karpv1.NodeClaim) error {
	ctx = log.IntoContext(ctx, log.FromContext(ctx).WithValues("nodeClaimName", nodeClaim.Name))
...
func (c *CloudProvider) instanceToNodeClaim(ctx context.Context, vm *armcompute.VirtualMachine, instanceType *cloudprovider.InstanceType) (*karpv1.NodeClaim, error) {
	...
	if zone, err := utils.GetZone(vm); err != nil {
		log.FromContext(ctx).V(-1).Info("failed to get zone for VM, zone label will be empty", "vmName", *vm.Name, "error", err)
	} else {
