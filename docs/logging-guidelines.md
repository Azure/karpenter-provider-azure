...
### Key Naming
- **Camel Case**: Use camel case for all logging keys (e.g., `instanceType`, `driftType`, `providerID`)
...
### Common Key Examples
- **Azure Resources**: `vmName`, `nicName`, `resourceGroupName`, `loadBalancerName`, `extensionName`
- **Azure Resource IDs**: `vmID`, `nicID`, `extensionID`
- **Kubernetes Resources**: `Node`, `NodeClaim`, `NodePool`
- **Versioning**: `currentKubernetesVersion`, `discoveredKubernetesVersion`, `expectedKubernetesVersion`, `actualKubernetesVersion`
- **Drift Detection**: `driftType`, `nodeClassHash`, `nodeClaimHash`, `actualImageID`
- **Operations**: `capacityType`, `instanceType`, `goalHash`, `actualHash`
- **Counts**: `instanceTypeCount`, `skuCount`, `loadBalancerCount`
- **Identity**: `expectedKubeletIdentityClientID`, `actualKubeletIdentityClientID`
