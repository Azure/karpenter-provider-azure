...
Self-hosted Karpenter supports Azure CNI Pod Subnet only with the default `aksscriptless` provision mode. It is not supported with `bootstrappingclient`, `aksmachineapi`, or `aksmachineapiheaderbatch`. Managed NAP support is controlled by AKS and is not implied by this self-hosted feature.

The AKS cluster must already have a pod subnet and allocation mode configured. Set `settings.podSubnetID` and `settings.podIPAllocationMode` to that cluster-level pair. Valid modes are `DynamicIndividual` and `StaticBlock`. Changing these cluster settings affects only newly provisioned nodes.

For values generated by an older `configure-values.sh`, move `POD_SUBNET_ID` and `POD_IP_ALLOCATION_MODE` from `controller.env` to `settings.podSubnetID` and `settings.podIPAllocationMode`. Remove the old env entries before changing the settings, since explicit `controller.env` entries take precedence.

An `AKSNodeClass` can select another pod subnet in the same VNet as its node subnet. Specify `podSubnetID` and `podIPAllocationMode` together, or omit both to inherit the cluster defaults. The pod subnet must differ from the node subnet. Azure permits only one allocation mode per pod subnet.

Pod-subnet networking puts the node and pod subnet names, VNet name, and resource group name into Kubernetes labels without alteration. These names must be at most 63 characters and start and end with an alphanumeric character; interior characters may also include `-`, `_`, and `.`. Names that Azure permits but Kubernetes labels cannot represent, such as resource groups containing parentheses, cause `SubnetsReady=False` with reason `SubnetIDInvalid`.

`configure-values.sh` cannot choose defaults for a pod-subnet cluster whose agent pools have different node subnet, pod subnet, or allocation-mode settings. In that case, configure the deployment values manually for the intended default tuple and set explicit pairs on NodeClasses that use other pod subnets. The script rejects pod-subnet clusters in unsupported provisioning modes rather than generating values without their networking settings.

```yaml
# Inherit the cluster pod subnet and mode.
spec: {}
```

```yaml
# Override both. Supplying only one field is rejected.
spec:
  podSubnetID: /subscriptions/.../virtualNetworks/.../subnets/pods-static
