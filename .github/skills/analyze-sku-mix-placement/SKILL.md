...
- `compared SKU Mix Placement recommendations with local ranking` from
  `stages/sku_mix_rank.go`, with `capacityType`, `capacityLimits`, `placementScope`, `splitID`,
  `localRanking`, and `recommendedRanking`.
...
When raw API responses are present, associate a ranking comparison by matching `splitID` to a
placement choice ID. Raw responses are not required for ranking analysis because comparison records
contain the placement choice ID and capacity limits. Do not group unrelated reconciles merely because
...
- Include the comparison record's `splitID` as the placement choice UUID in each ranking
  summary example, alongside the call number and NodeClaim details.
...
1. Include the NodeClaim name and `reconcileID` at call level. For every capacity type and scope,
  include that comparison record's `splitID` as its placement choice UUID. Do not select one
  comparison UUID to represent a call containing multiple scoped requests; mark only the affected
  comparison unavailable when its `splitID` is absent.
...
3. Report local-versus-recommended results separately for every observed capacity type and scope.
   ...
   - Immediately after every differing comparison, include a compact JSON block copied from the
     comparison record with `capacityType`, `placementScope`, `localRanking`, `recommendedRanking`,
     and `capacityLimits`. This should show the requested ranking, the recommendation returned by the
     placement provider, and the limits used for that comparison in one JSON object. Omit this block
     for complete matches. Do not include the full placement API response for ranking differences.
...
Example: Call 3 (..., NodeClaim `example-nodeclaim`, placement choice UUID
...
- `Standard_D_v3` family filtered entirely in zones 2, 3; observed in 18 calls. Example size
  `Standard_D2_v3`. Example: Call 2 (..., NodeClaim `example-nodeclaim`, placement choice UUID
...
NodeClaim: `example-nodeclaim`; reconcileID: `example-reconcile-id`

**Placement API response order vs expected priority order:** ✅ Correct

Placement choice UUID (on-demand, zonal): `example-placement-choice-id`
