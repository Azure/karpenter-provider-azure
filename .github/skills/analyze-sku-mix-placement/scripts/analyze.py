...
def ranking_evidence(record: Record) -> str:
    ...
    field_names = ("localRanking", "recommendedRanking", "capacityLimits")
    for field_index, field_name in enumerate(field_names):
        ...
        field_suffix = "," if field_index < len(field_names) - 1 else ""
...
def render_call(call: Call, number: int) -> str:
    ...
    if failures or unattached_errors:
        ...
        lines.append("**Placement API response order vs expected priority order:** ✅ Correct.")
    lines.append("")

    for comparison in sorted(call.comparisons, key=comparison_sort_key):
        label = scope_label(comparison)
        choice_id = comparison_choice_id(comparison) or "unavailable"
        lines.append(f"Placement choice UUID ({label}): `{choice_id}`")
        lines.append("")
...
def comparison_choice_id(comparison: Record | None) -> str | None:
    if comparison is None or comparison.data.get("splitID") in (None, ""):
        return None
    return str(comparison.data["splitID"])
...
def example_label(
    ...
    if choice_id := comparison_choice_id(comparison) or first_choice_id(response):
        details += f", placement choice UUID `{choice_id}`"
...
def render_summary(calls: list[Call]) -> str:
    ...
    ranking_groups: dict[
        str,
        dict[tuple[Any, ...], list[tuple[int, Call, Record, Response | None, str, str | None]]],
    ...
    for number, call in numbered_calls:
        ...
        for comparison in call.comparisons:
            ...
            for key, (description, example_size) in summary_issues.items():
                ranking_groups[scope][key].append(
                    (number, call, comparison, response, description, example_size)
                )
    ...
    for scope in all_scopes:
        ...
        for occurrences in unique_issues.values():
            number, call, comparison, response, description, example_size = occurrences[0]
            occurrence_count = len({number for number, _, _, _, _, _ in occurrences})
            ...
            lines.append(
                ...
                f"Example: {example_label(number, call, response, comparison)}."
            )
        lines.append("")
