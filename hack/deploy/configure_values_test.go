...
func TestConfigureValues(t *testing.T) {
	...
	dynamicPool := agentPool{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet, PodIPAllocationMode: &dynamic}
	staticPool := agentPool{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet, PodIPAllocationMode: &static}
	noPodPool := agentPool{VnetSubnetID: nodeSubnet}
	wantDynamic := networkSettings{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet, PodIPAllocationMode: dynamic, VnetGUID: "fake-vnet-guid"}
	wantStatic := networkSettings{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet, PodIPAllocationMode: static, VnetGUID: "fake-vnet-guid"}
	...
	tests := []testCase{
		...
		{name: "null allocation mode", pools: []agentPool{{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet}}, want: wantDynamic},
		{name: "empty allocation mode", pools: []agentPool{{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet, PodIPAllocationMode: &empty}}, want: wantDynamic},
		{name: "equivalent legacy dynamic modes", pools: []agentPool{{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet}, {VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet, PodIPAllocationMode: &empty}, dynamicPool}, want: wantDynamic},
		{name: "equivalent static pools", pools: []agentPool{staticPool, staticPool}, want: wantStatic},
		{
			...
			pools: []agentPool{{VnetSubnetID: strings.ToUpper(nodeSubnet), PodSubnetID: strings.ToUpper(podSubnet), PodIPAllocationMode: &lowerDynamic}, dynamicPool},
			want:  networkSettings{VnetSubnetID: strings.ToUpper(nodeSubnet), PodSubnetID: strings.ToUpper(podSubnet), PodIPAllocationMode: lowerDynamic, VnetGUID: "fake-vnet-guid"},
		},
		{
			name:  "case equivalent static pools preserve selected tuple",
			pools: []agentPool{{VnetSubnetID: strings.ToUpper(nodeSubnet), PodSubnetID: strings.ToUpper(podSubnet), PodIPAllocationMode: &lowerStatic}, staticPool},
			want:  networkSettings{VnetSubnetID: strings.ToUpper(nodeSubnet), PodSubnetID: strings.ToUpper(podSubnet), PodIPAllocationMode: lowerStatic, VnetGUID: "fake-vnet-guid"},
		},
	...
	for _, mismatch := range []struct {
		...
		{name: "node subnet", pool: agentPool{VnetSubnetID: nodeSubnet + "-other", PodSubnetID: podSubnet, PodIPAllocationMode: &dynamic}},
		{name: "pod subnet", pool: agentPool{VnetSubnetID: nodeSubnet, PodSubnetID: podSubnet + "-other", PodIPAllocationMode: &dynamic}},
		{name: "allocation mode", pool: staticPool},
		{name: "pod-free pool", pool: noPodPool},
	} {
		tests = append(tests,
