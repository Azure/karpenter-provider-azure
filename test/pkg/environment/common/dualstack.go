...
// DualStackServiceForDeployment creates a RequireDualStack Service for a deployment.
...
// HasIPv4AndIPv6PodIPs reports whether podIPs contains at least one address from each IP family.
...
// HasIPv4AndIPv6NodeInternalIPs reports whether a node has an InternalIP from each IP family.
...
// EventuallyExpectDualStackServiceConnectivity validates the dual-stack Service contract and
// verifies connectivity from the Linux system pool and any additional platform-specific probes.
...
func hasIPv4AndIPv6Addresses(addresses []string) bool {
	...
	for _, address := range addresses {
		...
		hasIPv4 = hasIPv4 || parsed.Is4()
		hasIPv6 = hasIPv6 || parsed.Is6()
	}
