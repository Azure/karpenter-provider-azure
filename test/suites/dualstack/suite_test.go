...
var _ = Describe("DualStack", func() {
	BeforeEach(func() {
		// Skip if the cluster is not dual-stack — check if kube-dns has multiple ClusterIPs
		dnsSvc, err := env.KubeClient.CoreV1().Services("kube-system").Get(env.Context, "kube-dns", metav1.GetOptions{})
		Expect(err).ToNot(HaveOccurred())
		if len(dnsSvc.Spec.ClusterIPs) < 2 {
			Skip("Skipping: cluster is not dual-stack (kube-dns has only one ClusterIP)")
		}
	})
