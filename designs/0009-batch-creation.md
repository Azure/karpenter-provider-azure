...
promise's wait function to use the GET-based poller (see design 0008) for per-machine
status tracking.
...
- **Instance promise WaitGroup (0007)**: With batching, `Create()` returns even earlier
...
- **GET-based poller (0008)**: Individual machines need their own polling mechanism since
