...
type Provisioner struct {
	...
	clock          clock.Clock
	podInjector    PodInjector // Hook for injecting fake pods for proactive scale-up
}
...
func NewProvisioner(kubeClient client.Client, recorder events.Recorder,
	...
	p := &Provisioner{
		...
		clock:          clock,
		podInjector:    &DefaultPodInjector{}, // Default to no-op injector
	}
	return p
}

// SetPodInjector sets a custom pod injector for proactive scale-up
...
func (p *Provisioner) GetPendingPods(ctx context.Context) ([]*corev1.Pod, error) {
	...
	// Inject fake pods for proactive scale-up (if injector is configured)
	// This allows providers to add fake pods based on workload definitions
	// (Deployments, Jobs, etc.) before real pods become pending
	pods = p.podInjector.InjectPods(ctx, pods)
